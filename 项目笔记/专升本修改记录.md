...
![QQ图片20240424110158](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404241102321.jpg)

查看服务器的事件查看器，有一个错误：

![QQ图片20240424110244](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404241102400.jpg)

不知道具体是什么情况，猜测是数据库连接出了问题。

后删除 iis 中其他的站点，将当前 站点重建，就好了。

新建的站点中，需要注意，应用程序池要启用无托管代码：

![image-20240425080303057](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404250803158.png)

# 2024.4.27

如果将 windows 上的 mysql 服务停止后，修改了 mysql 的配置文件 my.ini，然后重启 mysql 服务的重启不成功的话，弹出以下提示框：

![image-20240427085750092](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404270857227.png)

这种时候多半是因为 my.ini 保存的时候默认按 utf-8 的编码方式保存了，mysql 无法正常识别，所以将 my.ini 另存为，用 ansi 的编码方式保存即可。

方法来自[链接](https://www.zhihu.com/question/38891136)评论区。

# 发布在CentOS8上

在 VS 中打包发布的时候，要选择 Linux 的平台。
...
```bash
# 这里的路径最好是写完整，如果要到其他目录解压的话
DOTNET_FILE=/FTP/dotnet-sdk-6.0.421-linux-x64.tar.gz

# 在选好解压位置后执行，这个路径就是当前路径下的 .dotnet 路径
# 如果该目录不存在，则会创建它。
...
官网教程：https://learn.microsoft.com/zh-cn/dotnet/core/install/linux-scripted-manual#manual-install

当前的环境变量只在当前会话有用，将他设置为每个：

```bash
```

https://www.freecodecamp.org/chinese/news/how-to-set-an-environment-variable-in-linux/

## 验证码不出来

打开控制台发现，是找不到验证码的这个字体：

![image-20240427111552135](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404271120601.png)

在 /usr/share/fonts 下新建一个文件夹 arial，将 windows 下的字体文件拖过来即可

![image-20240427112021597](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404271120613.png)
...
![image-20240427112226450](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404271122558.png)

这是因为，在代码中，表名都是大写开头的，而在 windows 下，数据库的表名不区分大小写，都是小写的，但是在 Linux 中，数据库的表名是区分大小写的，所以要将服务器中的数据库的表名改为大驼峰命名：

![image-20240427134026795](https://gitee.com/LowProfile666/image-bed/raw/master/img/202404271340941.png)

## 持久运行

在 /etc/systemd/system/ 下编写一个 zsb.service 文件：

```txt
[Unit]  
Description=Zsb Managment System  
After=network.target  

[Service] 
User=root
Type=simple  
WorkingDirectory=/root/zsb
ExecStart=dotnet /root/zsb/ZsbManagment.dll 
Restart=always  
RestartSec=3  
...
+ 如果不指定根目录 WorkingDirectory 的话，根目录是 /，就会找不到静态资源

```shell
systemctl daemon-reload  # 刷新配置文件
systemctl start zsb  # 启动项目
systemctl enable zsb  # 开机自启
```
...
![image-20240501113643840](https://gitee.com/LowProfile666/image-bed/raw/master/img/202405011137887.png)

```
System.PlatformNotSupportedException: System.Drawing.Common is not supported on non-Windows platforms. See https://aka.ms/systemdrawingnonwindows for more information.
```

这是因为 System.Drawing.Common 在除了 Windows 外的系统上不支持 .NET 6了。解决这个问题可以在发布生成的文件夹中找到 ZsbManagment.runtimeconfig.json 文件，在里面添加一句：

```
"System.Drawing.EnableUnixSupport": true
```

![image-20240507172330698](https://gitee.com/LowProfile666/image-bed/raw/master/img/202405071723852.png)

然后再重新运行，会报错：

![image-20240501113706518](https://gitee.com/LowProfile666/image-bed/raw/master/img/202405011137901.png)

```
System.DllNotFoundException: Unable to load shared library 'libgdiplus' or one of its dependencies. In order to help diagnose loading problems, consider setting the LD_DEBUG environment variable: liblibgdiplus: cannot open shared object file: No such file or directory
```

这个错误表明系统中缺少了一个名为 `libgdiplus` 的共享库或其依赖项之一，而这个库是 System.Drawing.Common 在 Linux 上的一个实现所需的。缺少这个库会导致 FastReport 在初始化过程中抛出异常。

所以要在服务器上安装 libgdiplus：
...
```
No match for argument: libgdiplus
Error: Unable to find a match: libgdiplus
```

说明在 CentOS 的默认软件源中找不到 libgdiplus 包。使用其他来源的软件包：
...
![image-20240501140742185](https://gitee.com/LowProfile666/image-bed/raw/master/img/202405011407317.png)

但是如图，中文字体并没有正常显示出来。

## 解决中文乱码

将报表中用到的中文字体（宋体）安装到服务器上，Windows 上的字体文件在：C:\Windows\fonts 中，然后在服务器下 /usr/share/fonts/ 下创建一个文件夹 simsun，将 windows 上的宋体文件上传到服务器的 simsun 文件夹下。
...
![image-20240507172905655](https://gitee.com/LowProfile666/image-bed/raw/master/img/202405071729788.png)

参考资料

+ https://www.fastreportcn.com/post/2011.html
+ https://blog.csdn.net/secretdaixin/article/details/129277697
+ https://www.fastreportcn.com/post/2011.html
+ 
