  ...
  - 通过ServletContext对象也是可以记录日志的
    ...
    31-Dec-2023 23:04:20.097 严重 [http-nio-8080-exec-1] org.apache.catalina.core.ApplicationContext.log 未成年
    	java.lang.RuntimeException: 小屁孩
    		...
    		at org.apache.catalina.connector.CoyoteAdapter.service(CoyoteAdapter.java:338)
...
- get请求发送数据的时候，数据会挂在URI的后面，并且在URI后面添加一个“?”，"?"后面是数据。这样会导致发送的数据回显在浏览器的地址栏上。（get请求在“请求行”上发送数据）
  ...
  比如：http://localhost:8080/servlet05/getServlet?username=zhangsan&userpwd=1111
...
- HttpServletRequest接口中有哪些常用的方法？
  ...
  - 怎么获取前端浏览器用户提交的数据？以上的4个方法，和获取用户提交的数据有关系。
  ...
  - 手工开发一个webapp。测试HttpServletRequest接口中的相关方法。
    ...
    - 先测试了4个常用的方法，获取请求参数的四个方法。
      ...
      - `Map<String,String[]> parameterMap = request.getParameterMap();`：获取参数的Map集合

      - `Enumeration<String> names = request.getParameterNames();`：获取所有参数的名字（参数集合的key）

      - `String[] values = request.getParameterValues("name");`：获取到对应参数的值的一维数组

      - `String value = request.getParameter("name");`：获取对应参数的值的一维数组的第一个元素
...
public class RequestTestServlet extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        while (it.hasNext()) {
            ...
            out.print("name:" + Arrays.toString(value) + "<br>");
        }

        // 使用getParameterNames方法直接获取这个参数集合的所有的key
        Enumeration<String> names = request.getParameterNames();
        while (names.hasMoreElements()) {
            out.print(names.nextElement() + "<br>");
        }

        // 使用getParameterValues方法直接获取到对应参数的值的一维数组
        String[] values = request.getParameterValues("username");
        for (String v : values) {
            out.print(v + "<br>");
        }

        // 使用getParameter方法获取对应参数的值的一维数组的第一个元素
...
- 跳转（在 AServlet 中设置属性，想在 BServlet 中的请求域中取出来使用。）
  ...
  - 转发（一次请求）
    ...
    public class AServlet extends HttpServlet {
        @Override
        protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            // 在 AServlet 中设置属性，想在 BServlet 中的请求域中取出来使用。
            request.setAttribute("sysTime", new Date());
    
            // 以下代码虽然能实现这个功能，但是 Servlet 不能由程序员来创建，这样不受 Tomcat 管理
            /*
            ...
            */
            ...
            // 使用Servlet中的转发机制
            // 执行了AServlet后，跳转到BServlet。这个资源跳转可以使用转发机制完成
            // 怎么转发？
            // 第一步：获取请求转发器对象
            // 相当于把 “/b” 这个路径包装到了请求转发器当中，实际上是把下一个跳转的资源的路径告知给Tomcat服务器了。
            RequestDispatcher dispatcher = request.getRequestDispatcher("/b");// 参数是一个路径，BServlet的路径就是web.xml中配置的路径
...
- 转发的下一个资源必须是一个Servlet吗？
  ...
  - 注意：转发的时候，路径的写法要注意，转发的路径以“/”开始，不加项目名。
    ...
    ```java
    // 转发到一个html页面，只要是web容器中合法的资源都可以转发
...
- 实现步骤
  ...
  - 第四步：在IDEA当中搭建开发环境
    ...
    - JDBC的工具类
      ...
      ```java
      package zhang.shi.mao.oa.utils;
    ...
    - 第二：编写web.xml文件
      ...
      - ```xml
        <servlet>
            ...
            <servlet-class>zhang.shi.mao.oa.web.action.DeptListServlet</servlet-class>
        </servlet>
        <servlet-mapping>
            <servlet-name>list</servlet-name>
...
- 我们的第一个注解：
  ...
  - 在Servlet类上使用：@WebServlet，WebServlet注解中有哪些属性呢？
    ...
    - initParames属性：用来指定该servlet的初始化参数，该属性的值是一个注解数组 `@WebInitParam[]` ，这个 `@WebInitParam` 注解里面有两个值 `name` 和 `value`。等同于 <init-param>
    
    - value属性：这个属性和urlPatterns属性一样，都是用来指定Servlet的映射路径的。当注解的属性名是value的时候，使用注解的时候，value属性名是可以省略的。
    
      ```java
      // @WebServlet(urlPatterns = {"/welcome", "/welcome1"})
      // @WebServlet(urlPatterns = "/welcome")
      // @WebServlet(value = {"/welcome", "/welcome1"})
    ...
    - 注意：不是必须将所有属性都写上，只需要提供需要的。（需要什么用什么。）
      ...
      @WebServlet(name = "hello", 
                  urlPatterns = {"/hello", "/hello2"}, 
                  loadOnStartup = 1,
                  initParams = {
                      @WebInitParam(name="username", value="root"), 
                      @WebInitParam(name = "passwrod", value = "1234")
                  })
      public class HelloServlet extends HttpServlet {
          public HelloServlet() {
...
public class ReflectAnnotation {
    public static void main(String[] args) throws Exception{
        // 使用反射机制，拿到类上面的注解进行解析
        // 获取Class对象
        Class<?> welcomeServlet = Class.forName("zhang.shi.mao.servlet.WelcomeServlet");

        // 获取这个类上面的注解对象
        // 先判断这个类上面有没有注解对象，有则获取
        // boolean annotationPresent = welcomeServlet.isAnnotationPresent(WebServlet.class);
        // System.out.println(annotationPresent);

        if (welcomeServlet.isAnnotationPresent(WebServlet.class)) {
            // 获取注解对象
            WebServlet welcomeServletAnnotation = welcomeServlet.getAnnotation(WebServlet.class);
            // 获取注解对象的value属性值
            String[] value = welcomeServletAnnotation.value();
            for (String s : value) {
...
public class DeptServlet extends HttpServlet {
    ...
    // 为什么不重写doGet和doPost？
    // 		因为不知道前端发送的请求是get还是post，所以如果要重写的需要在doGet里面调用doPost，
    //		在doPost里也要调用doGet方法，麻烦。
    //		所以直接重写doGet和doPost的上一级方法service，因为doGet和doPost是由service方法调用的，
    //		重写了service就不用管前端的请求方式了
...
以上代码中，如果使用模糊匹配方式：`@WebServlet("/dept/*")`，在获取 servlet 路径的时候：`request.getServletPath()`，获取到的只有 `/dept`，可以使用 `request.getRequestURI()` 方法代替，这个方法会获取到整个 URI，比如 `/servlet11/dept/list` ，然后可以使用分割字符串在进行比对。
...
- 思考一下：session对象的实现原理。
  ...
  ![session对象的实现原理](https://gitee.com/LowProfile666/image-bed/raw/master/img/202401092143719.png)
...
- session的实现原理：
  ...
  第一次访问的时候，服务器会给浏览器发一个标志，然后后面再访问，就会通过这个标志去获取服务器的 session 对象：

  ![image-20240109155657118](https://gitee.com/LowProfile666/image-bed/raw/master/img/202401091556265.png)
...
- 销毁session对象：
  ...
  - 可以在 web.xml 文件中配置 session 超时销毁的时间
...
- 在java的servlet中，对cookie提供了哪些支持呢？
  ...
  - java程序怎么把cookie数据发送给浏览器呢？response.addCookie(cookie);
    ...
    ```java
    // 创建Cookie对象
    Cookie cookie = new Cookie("productId", "12344556");
    
    // 将Cookie响应到浏览器上
    response.addCookie(cookie);
    ```

- 在HTTP协议中是这样规定的：当浏览器发送请求的时候，会自动携带该path下的cookie数据给服务器。（通过URL。）
...
- 将上面的项目部署之后，启动服务器，打开浏览器，访问以下地址：
  ...
  - 在启动项目后，控制台会有输出：
    `CATALINA_BASE:     C:\Users\20620\AppData\Local\JetBrains\IntelliJIdea2023.2\tomcat\303776fe-359f-4f6c-9a9c-469d25bd810c`，在这个目录下的 `work\Catalina\localhost\jsp\org\apache\jsp` 的文件夹里就会生成 index_jsp.java 文件和 index_jsp.class 文件。
...
- 怎么在JSP中编写Java程序：
  - <% java语句; %>
    ...
    - 这里你要细心点，你要思考，在<% %>这个符号里面写java代码的时候，你要时时刻刻的记住你正在“方法体”当中写代码，方法体中可以写什么，不可以写什么，你心里是否明白呢？
      ...
      以上代码会正常生成 Java 源代码，但是编译会出错，访问这个jsp文件会报 500 错误。因为这是 Java 的语法错误，不能在方法体中声明成员变量。
  ...
  - <%-- --%>
    ...
    - jsp 的专业注释，注释里面的内容不会被翻译到Java源文件中。
    
    - 在jsp中不要使用HTML的注释：<!-- -->，该注释中的内容会被翻译到Java源文件中。
  ...
  - <%! %>
    ...
    - 在这个符号当中编写的java程序会自动翻译到service方法之外：
      ...
      ![image-20240106090820312](https://gitee.com/LowProfile666/image-bed/raw/master/img/202401060908385.png)
  
    - 这个语法不管是写在 `<%%>` 这个符号的上面还是下面，最终的翻译的代码都是在类体当中，而不是在 service 方法中，只有相同的符号才会有顺序。
  ...
  - JSP的输出语句
    ...
    - 注意：以上代码中的out是JSP的九大内置对象之一。可以直接拿来用。当然，必须只能在service方法内部使用，也就是只能写在 <%%> 符号中。
    ...
    - 如果输出的内容中含有“java代码”，这个时候可以使用以下语法格式：
      - <%= %> 注意：在=的后面编写要输出的内容。
        ...
        ![image-20240106092853371](https://gitee.com/LowProfile666/image-bed/raw/master/img/202401060928421.png)
...
- EL表达式出现在JSP中主要是：
  - 从某个作用域中取数据，然后将其转换成字符串，然后将其输出到浏览器。这就是EL表达式的功效。三大功效：
    - 第一功效：从某个域中取数据。
      ...
      - 经过测试，如果几个域中有多个同名的属性，会按域的大小从小到大取：
      
        ```jsp
        <%
            // 向request域中存储username为zhangsan
            request.setAttribute("username", "zhangsan");
            pageContext.setAttribute("username", "lisi");
            session.setAttribute("username", "wangwu");
        %>
        
        <%--使用Java代码取出username的值并显示在浏览器上--%>
        ...
        <hr>
        <%--使用EL表达式--%>
        <%--如果有pageContext存在，那么pageContext最小，EL表达式的值一直是lisi-->
        <%--如果没有pageContext存在，那么request最小，EL表达式的值一直是张三-->
        ${username}
        <hr>
        ${username}
        <hr>
...
- EL表达式的使用：
  ...
  - ```jsp
    ...
    你不要这样写：${"userObj"}，这样写会直接输出一个字符串 "userObj"
    ...
    注意：如果没有对应的get方法，则出现异常。报500错误。可以没有对应的属性，但是一定要有get方法
  ...
  - EL表达式优先从小范围中读取数据（在没有指定范围的情况下）。
  
    - pageContext < request < session < application
  
  - 在EL表达式中可以指定范围来取数据，EL表达式中有四个隐含的隐式的范围：
  ...
  - EL表达式取数据的时候有两种形式：
    ...
    ```jsp
    <%@ page import="zhang.shi.mao.User" %>
    <%
        User user = new User();
        user.name = "zhangsan";
  ...
  - 掌握使用EL表达式，怎么从数组和List集合中取数据：
    ...
    - ${数组[0]}，就算是下标越界，也不会出错，只不过是前端页面不会有数据显示
    - ${数组[1]}
    - ${list[0]}
  ...
  - 通过EL表达式获取应用的根：
    ...
    <%--
        ...
        其中有四个域对象，最小的域是 pageContext
        pageContext翻译为：页面上下文，
    --%>
    <%--以下代码的结果是一样的，pageContext.getRequest()是获取request对象的，而JSP中自带了request对象，可以直接使用--%>
    <%=pageContext.getRequest()%><br>
    <%=request%>
    <hr>
    
    <%--在EL表达式中没有request这个隐式对象，
        requestScope只代表请求范围，不等同于request对象，
        但在EL表达式中有个隐式的对象 pageContext，
        这个pageContext对象和JSP九大内置对象中的pageContext对象是同一个，
    --%>
    <%=pageContext.getRequest()%>这段Java代码就可以转为EL表达式：
    ${pageContext.request}
    <hr>
  ...
  - EL表达式中其他的隐式对象：
    ...
    <%--EL表达式的内置隐式对象--%>
    <%--param--%>
    <%--请求的路径：http://localhost:8080/jsp/1.jsp?username=zhangsan--%>
    用户名：<%=request.getParameter("username")%><br>
    用户名：${param.username}<br>
    
    <%--请求的路径：http://localhost:8080/jsp/1.jsp?username=zhangsan&hobby=smoke&hobby=drink&hobby=rap--%>
    <%--param是获取请求参数一维数组中的第一个元素--%>
    爱好：${param.hobby}<br>
    
    <%--paramValues--%>
    <%--这个方式拿到是一个一维数组--%>
    爱好：${paramValues.hobby}<br>
    爱好：<%=request.getParameterValues("hobby")%><br>
    ...
    ```jsp
    <!--    配置Servlet上下文初始化参数-->
    <!--    上下文初始化参数被封装到ServletContext对象当中-->
    <context-param>
        <param-name>pageSize</param-name>
    ...
    ----------------------------------
    <%--initParam--%>
    <%--ServletContext是Servlet上下文对象，对应的是JSP中九大内置对象之一：application--%>
    <%
  ...
  - EL表达式的运算符
    ...
    - 算术运算符
      ...
      - EL表达式中的 `+` 只会求和，不会进行字符串拼接；两边不是数字时，一定会转成数字，转不成数字就报错 
      
        ```jsp
        ${10 + 20}<br>
        ${10 + "20"}<br>
        \${10 + "abc"}<br> <%--java.lang.NumberFormatException: For input string: "abc"--%>
        \${"abc" + "def"}
        ```
...
- 使用JSTL标签库的步骤：
  ...
  - 第三步：在需要使用标签的位置使用即可。表面使用的是标签，底层实际上还是java程序。
    ...
    <%
        ...
        User u1 = new User("zs", 13);
        User u2 = new User("li", 24);
        list.add(u1);
        list.add(u2);
    ...
    <%
        ...
        for (User u : users) {
    %>
        name:<%=u.name%>, age:<%=u.age%><br>
    <%
        }
    %>
    <hr>
    <%--使用core标签库的foreach标签，对list集合进行遍历--%>
    <%--EL表达式只能从域中取数据--%>
    <%--var后面的名字随意，var 属性代表的是集合中的每一个元素--%>
...
- JSTL标签的原理
  ...
  - jstl中的核心标签库core当中有哪些常用的标签呢？
    ...
    - c:if
      ...
      <%--if标签还有var属性，不是必须的--%>
      <%--if标签还有scope属性，不是必须的，用来指定var的存储域
          scope有四个选项可以选：page(pageContext域)、request(request域)、session(session域)、application(application域)
      --%>
      <%--将var属性中的v存到request域中--%>
      <c:if test="${not empty param.username}" var="v" scope="request">
          <h1>欢迎你${param.username}！</h1>
      </c:if>
      
      <%--将上面存的v取出来看看
          这个v是保存的test属性中的条件的值true或false
      --%>
      ${v}
      ```
    ...
    - c:forEach
      ...
      <%--varStatus="这个属性表示var的状态对象，这里是一个Java对象，这个Java对象代表了var的状态"
          varStatus="这个属性名字是随意的"，这个状态对象有一个count属性，可以直接使用,
          这个count值从1开始，以1递增，主要用于编号/序号
      --%>
      <c:forEach items="${users}" var="u" varStatus="uStatus">
          编号：${uStatus.count}, name:${u.name}, age:${u.age}<br>
      </c:forEach>
      ```
    ...
    - c:choose c:when c:otherwise
      ...
      - ```
        ...
        <c:choose>
            ...
            <c:when test="${param.age < 60}">中年</c:when>
            <c:otherwise>老年</c:otherwise>
        </c:choose>
        ```
        
      - 注意：test属性中不要写多余的空格，否则不会生效，像这样：`<c:when test="${param.age < 18} ">少年</c:when>`
...
- 一个过滤器怎么写呢？
  ...
  - 第一步：编写一个Java类实现一个接口：jarkata.servlet.Filter。并且实现这个接口当中所有的方法（也可也只重写init方法，因为其他两个方法是默认方法）。
  ...
  - 第二步：在web.xml文件中对Filter进行配置。这个配置和Servlet很像。
    ...
    - ```xml
      <filter>
          ...
          <filter-class>zhang.shi.mao.Filter1</filter-class>
      </filter>
      <filter-mapping>
          <filter-name>filter1</filter-name>
...
- 关于Filter的配置路径：
  ...
  /*以下这个路径属于模糊匹配中的扩展匹配，以星号开始，注意这种格式不要以 / 开始*/
  @WebFilter("*.do")
  ...
  public class Filter1 implements Filter {
      ...
      public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
              throws IOException, ServletException {
          // 在请求的时候添加过滤规则
          System.out.println("doFilter方法执行开始");
  
          // 执行下一个过滤器，如果下一个不是过滤器了则执行目标程序Servlet
  ...
  - jakarta.servlet.http包下：
    ...
    - HttpSessionAttributeListener
      ...
      public class MyHttpSessionAttributeListener implements HttpSessionAttributeListener {
          // 向session域中存储数据的时候，以下方法被web服务器调用
          @Override
          public void attributeAdded(HttpSessionBindingEvent se) {
              System.out.println("session add");
          }
      
          // 将session域中存储的数据删除的时候，以下方法被调用
          @Override
          public void attributeRemoved(HttpSessionBindingEvent se) {
              System.out.println("session remove");
          }
      
          // session域中的某个数据被替换的时候，以下方法被调用
          @Override
...
- 注意：所有监听器中的方法都是不需要javaweb程序员调用的，由服务器来负责调用？什么时候被调用呢？
  ...
  public class MyServletContextListener implements ServletContextListener {
      /**
       * 监听器的方法不需要程序手动调用，是发生某个特殊事件后被服务器调用
