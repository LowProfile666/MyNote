...
![image-20240608142203790](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608142203790.png)

+ 使用注解时，如果value是一个数组，且数组中只有一个元素，那么数组的大括号可以省略；
+ 使用注解时，如果只使用value一个属性的话，value也可以省略

且可以看到value属性上有一个别名叫做path，而path属性有一个别名是value，说明value属性也可以使用path来代替。
...
![image-20240608144929701](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608144929701.png)

![image-20240608144954466](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608144954466.png)
...
注意：`/x**z/` 实际上并没有使用通配符 `**`，本质上还是使用的 *，因为通配符 ** 在使用的时候，左右两边都不能有任何字符，必须是 /，否则就相当于一个 *。
...
```
在路径中使用了占位符 {}，就要使用 @PathVariable 注解来标注方法参数，这个注解有一个value属性和一个name属性，这两个属性是一样的：

![image-20240608151032758](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608151032758.png)

要在@PathVariable注解中的指定参数的名字，这个名字就是路径中占位符中的名字，它就会将对应的数据传给对应的参数。
...
```
在index.html文件中添加超链接，这是Thymeleaf模板语法：
```html
...
![image-20240608160310726](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608160310726.png)

使用  params = {"!username","password"} 参数和  params = {"password"} 效果是不一样的，前者不能包含username参数，但是后者可以包含username参数。

### 第三四种用法

```java
@GetMapping(value = "/login", params = {"username=admin","password!=123"})
public String login() {
    return "user/detail";
}
```

此时请求路径必须是get，且必须带有username和password参数，username参数的值必须是admin，password参数的值必须不是123：

![image-20240608160657750](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608160657750.png)

否则都不能访问到：

![image-20240608160719085](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608160719085.png)

![image-20240608160742167](https://gitee.com/LowProfile666/image-bed/raw/master/img/image-20240608160742167.png)
